...
test('municipal ガードでログインしても web ガードはログアウト状態', function () {
    $user = User::create([
        'name' => '役所ユーザー',
    ...
    auth()->guard('municipal')->login($user);

    expect(auth()->guard('municipal')->check())->toBeTrue();
    expect(auth()->guard('web')->check())->toBeFalse();
});
...
test('web ガードでログインしても admin ガードはログアウト状態', function () {
    ...
    expect(auth()->guard('municipal')->check())->toBeFalse();
});

test('admin と municipal は互いに独立している', function () {
    $admin = User::create([
        'name' => 'Admin',
        'email' => 'admin@example.com',
        'password' => 'password',
    ...
    auth()->guard('municipal')->login($municipal);
