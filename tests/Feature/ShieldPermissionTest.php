...
test('municipal ロールを持つユーザーは government パネルにアクセスできる', function () {
    $user = User::create([
        'name' => '役所ユーザー',
    ...
    expect($user->canAccessPanel(filament()->getPanel('government')))->toBeTrue();
});
...
test('worker ロールのユーザーは admin パネルにアクセスできない', function () {
    ...
    expect($user->canAccessPanel(filament()->getPanel('government')))->toBeFalse();
});
